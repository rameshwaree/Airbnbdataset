...
We obtained the Airbnb Dataset from gaggle(New York City 2019). This Dataset contains 16 variables as shown in QA. This dataset shows how the airbnb market was in NYC in 2019.
```{r}
colours <- c( "black", "#b56576","#e56b6f","#eaac8b" )
```
...
str(airbnb)
```
We have 48895 observation and 16 variabels we will be focusing on "neighbourhood_group ,latitude, longitude ,room_type ,price , minimum_nights , number_of_reviews ,availability_365  " for our different models 
```{r}
any(is.na(airbnb))
```
```{r}
...
unique(airbnb$price)
```
after removing the null and duplicate values. I also removed the negative and 0 values later because it was giving me -INf mean.
```{r}
...
```
#EDA
For EDA we explored different varibales in the relationship with the price.
```{r}
...
```{r}
bp <- boxplot.stats(airbnb$minimum_nights)

# Get outliers
outliers <- bp$out

# Remove outliers from your data
cleaned_data <- airbnb[!airbnb$minimum_nights %in% outliers, ]

# Create a boxplot of the cleaned data
boxplot(cleaned_data$minimum_nights, main = "Boxplot of minimum_nights without outliers")
```
...
```
we had outlier for the minimum night variable because of this variable the max was 1250.
...
```{r}
logged_prices <- c(2.303, 4.234, 4.615, 4.697, 5.136, 9.210)

# Convert logged prices to normal values
normal_prices <- exp(logged_prices)
...
```{r}
unique(airbnb$number_of_reviews)
...
XX <- barplot(Neighbourhood[orderIDX], ylim = yLimit, axes = FALSE, names.arg = FALSE, col = colours[2], border = colours[1])
...
## how does neighborhood characteristics, room types and other factors influence the accuracy of predictive models for Airbnb pricing than the one doesnt include the neighborhood characteristics, room types variable?

###NULL :There is no significant relationship between neighbourhood characteristics, room types, and other factors, and the accuracy of predictive models for Airbnb pricing.
###ALTERNATIVE : There is an significant relationship between neighbourhood Characteristic, room type and other factors and the accuracy of predictive models for Airbnb pricing
...
```
^^we are splitting the data 80 % training 20%testing 
...
```
^^ using stratified sampling to split the data into training, testing, and validation for the neighbourhood_group 
...
```{r}
theModel1 <- lm(log_price ~ neighbourhood_group + minimum_nights + number_of_reviews + availability_365, data = airbnb.Train)
summary(theModel1)
```
...
car::vif(theModel1)
```
as we can see there is very little multicollinearity  
...
```{r}
xLim1 <- range(pretty(range(airbnb.Train$log_price)))
yLim1 <- range(pretty(range(airbnb.Train[, theVariables[i]])))


xLim2 <- range(pretty(range(airbnb.Train2$log_price)))
yLim2 <- range(pretty(range(airbnb.Train2[, theVariables[i]])))


identical(xLim1, xLim2)  
identical(yLim1, yLim2)  


identical(par("mfcol"), c(2, 3))  
identical(par("mai"), c(0.55, 0.25, 0.1, 0.1))  
identical(par("omi"), c(0.3, 0.3, 0.3, 0.3))  

```
since the graph looked the same I checked if there were any changes on them after the cooks distance. 
...
for (i in 1:1000){
  ...
  theModel.temp <- lm( log_price ~ neighbourhood_group + minimum_nights + number_of_reviews + availability_365, data = sampleDF)
...
```
availibity is almost close to 0 
```{r}
theModel.Final <- lm(log_price ~ minimum_nights + longitude +latitude,data=airbnb.Train2)
```
...
```{r}
 theModel1 <- lm(log_price ~ neighbourhood_group + minimum_nights+ number_of_reviews + availability_365, data = airbnb.Train2)
summary(theModel)
```
```{r}
...
```
the neighborhood model is better.
```{r}
...
because there is a significant relationship between neighbourhood Characteristic, room type and other factors and the accuracy of predictive models for Airbnb pricing
